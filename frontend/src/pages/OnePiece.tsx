...
import { searchOpCards, getOpCardPrices, getOpCardSets } from '../api/onepiece'
import { useNavigate } from 'react-router-dom'
import { Search } from 'lucide-react'

const POPULAR_SEARCHES = [
  'Monkey D. Luffy', 'Roronoa Zoro', 'Nami', 'Sanji',
  'Nico Robin', 'Boa Hancock', 'Trafalgar Law', 'Portgas D. Ace',
]
...
function OpPriceChip({ cardId }: { cardId: string }) {
  const { data } = useQuery({
    queryKey: ['op-prices-chip', cardId],
    queryFn: () => getOpCardPrices(cardId),
    staleTime: 60_000,
  })
  ...
  return <span className="text-sm font-bold text-red-400">${price.price_usd.toFixed(2)}</span>
}
...
export default function OnePiece() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['op-search', debouncedQ],
    queryFn: () => searchOpCards(debouncedQ),
    enabled: debouncedQ.length >= 2,
  })

  const { data: allSets } = useQuery({
    queryKey: ['op-sets'],
    queryFn: getOpCardSets,
    staleTime: 300_000,
  })
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-white">One Piece Prices</h1>
        <p className="text-sm text-muted mt-1">One Piece TCG card market prices · PriceCharting data</p>
      </div>
      ...
      <form onSubmit={handleSearch} className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-surface text-white rounded-xl px-4 py-3 pl-9 text-sm border border-white/10 focus:border-red-400/50 outline-none"
            autoFocus
          />
          {q && (
        ...
        <button type="submit" className="px-6 py-3 bg-red-600 hover:bg-red-500 text-white text-sm font-semibold rounded-xl transition-colors">
          Search
        </button>
      </form>
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {POPULAR_SEARCHES.map((term) => (
            <button key={term} onClick={() => { setQ(term); setDebouncedQ(term) }}
              className="px-3 py-1.5 bg-surface border border-white/10 hover:border-red-400/40 rounded-full text-xs text-white transition-colors">
              {term}
            </button>
          ))}
        </div>
      ...
      {showResults && (
        isLoading ? (
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {cards.map((card) => (
                <div key={card.id} onClick={() => navigate(`/op/card/${card.id}`)}
                  className="bg-surface rounded-xl border border-white/5 overflow-hidden cursor-pointer hover:border-red-400/40 hover:shadow-lg transition-all group">
                  {/* Card image */}
                  ...
                  <div className="p-3">
                    ...
                    <p className="text-xs text-muted truncate mt-0.5">{card.set_code}</p>
          ...
          <div className="text-center py-16 text-muted">
            ...
            <p className="text-sm">Try a card name or set like &ldquo;OP07&rdquo;</p>
          </div>
        )
      )}
              ...
              {(allSets ?? [])
                ...
                .map((set) => (
                  <button key={set.set_code}
                    ...
                    className="bg-surface border border-white/5 hover:border-red-400/40 rounded-xl p-4 text-left transition-all hover:shadow-lg group">
                    <p className="text-sm font-semibold text-white group-hover:text-red-400 transition-colors truncate">{set.set_name}</p>
