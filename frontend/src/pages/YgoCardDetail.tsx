...
import { getYgoCard, getYgoCardPrices } from '../api/ygo'
...
export default function YgoCardDetail() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()

  const { data: card } = useQuery({ queryKey: ['ygo-card', id], queryFn: () => getYgoCard(id!) })
  const { data: prices, dataUpdatedAt } = useQuery({
    queryKey: ['ygo-card-prices', id],
    queryFn: () => getYgoCardPrices(id!),
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2 text-xs text-muted">
        <button onClick={() => navigate('/ygo')} className="hover:text-white transition-colors">Yu-Gi-Oh!</button>
        <span>/</span>
        ...
        <div className="flex gap-6 flex-col sm:flex-row">
          ...
          <div className="flex-shrink-0 flex justify-center">
            {card.image_small ? (
              <img
                ...
                alt={card.name}
                className="w-44 rounded-xl shadow-lg border border-yellow-400/20 object-contain"
              />
            ) : (
              <div className="w-44 h-60 bg-surface-2 rounded-xl flex flex-col items-center justify-center gap-2 border border-yellow-400/20">
                <div className="text-2xl">⚔️</div>
                <span className="text-xs text-yellow-400/60 font-semibold tracking-widest uppercase">Loading…</span>
              </div>
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {card.set_code && card.set_code !== card.set_name && (
              <p className="text-yellow-400/80 text-xs mt-0.5 font-mono">{card.set_code}</p>
            )}
            <div className="flex flex-wrap gap-1 mt-2">
              {card.rarity && <span className="px-2 py-0.5 bg-white/10 rounded text-xs text-white">{card.rarity}</span>}
              {card.supertype && card.supertype !== 'Card' && <span className="px-2 py-0.5 bg-yellow-400/10 text-yellow-300 rounded text-xs">{card.supertype}</span>}
              {card.attribute && <span className="px-2 py-0.5 bg-white/10 rounded text-xs text-white">{card.attribute}</span>}
              {card.race && <span className="px-2 py-0.5 bg-white/10 rounded text-xs text-white">{card.race}</span>}
            </div>
            {/* ATK / DEF */}
              ...
              <div className="flex gap-4 mt-3">
                {card.atk != null && (
                  <div className="text-center">
                    <div className="text-xs text-muted uppercase tracking-wide">ATK</div>
                    <div className="text-lg font-black text-yellow-400">{card.atk}</div>
                  </div>
                )}
                {card.def != null && (
                  <div className="text-center">
                    <div className="text-xs text-muted uppercase tracking-wide">DEF</div>
                    <div className="text-lg font-black text-yellow-400">{card.def}</div>
                  </div>
                )}
              </div>
            )}
            {/* Card description */}
            {card.desc && (
              <p className="text-xs text-muted mt-3 leading-relaxed line-clamp-4">{card.desc}</p>
            )}
